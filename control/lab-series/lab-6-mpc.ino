/*
Lab 6: MPC Concept (ESP32 as plant; Node-RED chooses u)
- Subscribes u from cps/ctrl/act
- Publishes y (ADC) and optional yhat (first-order filtered) to cps/ctrl/sensor
Node-RED runs candidate MPC and sends u.
*/
...
float alpha = 0.90f;   // filtered output parameter
float yhat = 0.0f;
...
void connectMQTT() {
  ...
  while (!mqtt.connected()) {
    String cid = "esp32-lab6-" + String((uint32_t)ESP.getEfuseMac(), HEX);
...
void loop() {
  ...
  if (now - lastMs >= TS_MS) {
    ...
    // Optional: filtered output based on control input (for model checking)
    yhat = alpha * yhat + (1.0f - alpha) * (float)u_cmd;
